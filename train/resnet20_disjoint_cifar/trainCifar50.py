...
def train(config):
    ...
    # %%
    CIFAR100_MEAN = [129.3105, 124.1085, 112.404]
    CIFAR100_STD = [68.2125, 65.4075, 70.4055]
    normalize = T.Normalize(np.array(CIFAR100_MEAN)/255, np.array(CIFAR100_STD)/255)
    denormalize = T.Normalize(-np.array(CIFAR100_MEAN)/np.array(CIFAR100_STD), 255/np.array(CIFAR100_STD))
